...
{
	$id = $_GET['edit_id'];
	$stmt_edit = $DB_con->prepare('SELECT descripcion,categoria_nombre,cantidad,estado,observaciones,mfecha_revision,Imagen_Img FROM mobiliario WHERE Imagen_ID =:uid');
	...
	if(!isset($errMSG))
	{
		$stmt = $DB_con->prepare('UPDATE mobiliario 
									 SET descripcion=:descripcion, 
										 ...
										 mfecha_revision=:mfecha_revision, 
										 Imagen_Img=:upic 
								   WHERE Imagen_ID=:uid');
...
<input type="number" id="tentacles" name="cantidad" value="<?=$cantidad?>" 
   ...
   <tr>
	  ...
	  <td><input class="fechamobili" type="date" align="center" placeholder="Fecha de revision"name="mfecha_revision" maxlength="70"  value="<?php echo $mfecha_revision; ?>"  /></td>
	  </tr>
			<br>     
